...
LAUNCHER_ENV  := sheaf-launcher.env
ENGINE_ENV    := sheaf-launcher-engine.env
BUN_APP       := /c/work/typescript/sheafbase2
...
$(shell [ -f $(LAUNCHER_ENV) ] || bun -e "const uuid=require('crypto').randomUUID();const lines=['# SheafBase launcher configuration','# Change SHEAF_LAUNCHER_UUID when launcher/engine protocol changes incompatibly','# Both sides must match or auth will fail','','SHEAF_LAUNCHER_UUID='+uuid,'','# Window settings','SHEAF_LAUNCHER_TITLE=$(APP_NAME)','SHEAF_LAUNCHER_WIDTH=1200','SHEAF_LAUNCHER_HEIGHT=800','','# Engine settings','SHEAF_LAUNCHER_PORT_RETRIES=3','SHEAF_LAUNCHER_READY_TIMEOUT=15'];require('fs').writeFileSync('$(LAUNCHER_ENV)',lines.join('\n')+'\n');console.error('[sheaflauncher] generated $(LAUNCHER_ENV) uuid='+uuid);")

$(shell [ -f $(ENGINE_ENV) ] || bun -e "const fs=require('fs');const e=fs.readFileSync('$(LAUNCHER_ENV)','utf8');const lines=e.split('\n');const uuidLine=lines.find(l=>l.startsWith('SHEAF_LAUNCHER_UUID='));const uuid=uuidLine?uuidLine.split('=')[1].trim():require('crypto').randomUUID();const out=['# SheafBase engine configuration','# SHEAF_LAUNCHER_UUID must match the Go launcher','# Change when launcher/engine protocol changes incompatibly','# Both sides must match or auth will fail','','SHEAF_LAUNCHER_UUID='+uuid];fs.writeFileSync('$(ENGINE_ENV)',out.join('\n')+'\n');console.error('[sheaflauncher] generated $(ENGINE_ENV)');")

# ── Read values from sheaf-launcher.env ──────────────────────────────────────
...
UUID    := $(call getenv,SHEAF_LAUNCHER_UUID)
TITLE   := $(call getenv,SHEAF_LAUNCHER_TITLE)
WIDTH   := $(call getenv,SHEAF_LAUNCHER_WIDTH)
HEIGHT  := $(call getenv,SHEAF_LAUNCHER_HEIGHT)
RETRIES := $(call getenv,SHEAF_LAUNCHER_PORT_RETRIES)
TIMEOUT := $(call getenv,SHEAF_LAUNCHER_READY_TIMEOUT)
...
# ── Build info header ─────────────────────────────────────────────────────────
$(info )
$(info [sheaflauncher] build environment)
$(info   app:    $(APP_NAME))
$(info   go:     $(GO_VERSION))
...
sync-env:
	...
	@echo "[sheaflauncher] synced $(ENGINE_ENV) to $(BUN_APP)"

## Assemble full distribution folder — builds everything and downloads bun.exe if needed
dist: build
	@echo "[sheaflauncher] assembling dist..."
	@mkdir -p dist
	@cp -r $(BUN_APP)/build/. dist/build/
	@echo "[sheaflauncher] copied engine build from $(BUN_APP)/build"
	@cp $(APP_NAME).exe dist/
	@echo "[sheaflauncher] copied launcher"
	@if [ ! -f dist/bun.exe ]; then \
        echo "[sheaflauncher] downloading bun.exe..."; \
        ...
        rm -f bun.zip; \
        echo "[sheaflauncher] bun.exe downloaded"; \
    else \
        echo "[sheaflauncher] bun.exe already present, skipping download"; \
    fi
	@echo "[sheaflauncher] dist ready:"
...
dist-linux: build
	@echo "[sheaflauncher] assembling dist-linux..."
	@mkdir -p dist-linux
